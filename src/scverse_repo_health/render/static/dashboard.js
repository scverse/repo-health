...
(() => {
	...
	function applyFilters() {
		...
		for (const row of rows) {
			const matchesText =
				!needle || row.dataset.repo.toLowerCase().includes(needle);
		...
		for (const section of sections) {
			const any = Array.from(section.querySelectorAll("tr[data-repo]")).some(
				(r) => !r.hidden,
			);
			section.hidden = !any;
		}
		...
		if (counter)
			counter.textContent = `${visible} of ${rows.length} repositories`;
	...
	function paintGroups() {
		...
		if (all) {
			const everyGroup = new Set(
				Array.from(matrix.querySelectorAll(".colgroup-toggle")).map(
					(b) => b.dataset.group,
				),
			);
			all.textContent =
				collapsed.size >= everyGroup.size
					? "Expand all groups"
					: "Collapse all groups";
		}
	}
	...
	$("collapse-all")?.addEventListener("click", () => {
		const everyGroup = Array.from(
			matrix.querySelectorAll(".colgroup-toggle"),

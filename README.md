...
The published artifact drops the raw file contents and API payloads the checks have already consumed, taking it from roughly 90 KiB per repo to 13 KiB.
`collect --no-slim` keeps everything, for debugging a check against real data.

Every check is a **pure function** of an already-fetched `RepoData`, so each one is unit-tested against a JSON fixture captured from a real repo.

A check that cannot be evaluated renders `?`, never a failure.
The tool still works without the GitHub App installed; it just knows less.
...
A check that applies but cannot be evaluated renders `?`, never a failure.
...
The last four are `–` for a repo with no `.readthedocs.yaml`, which is already the finding and would otherwise be counted five times over.
...
`trusted-publishing` is verified rather than assumed: the Sigstore certificate inside the attestation names the exact workflow and tag that published the artefact, e.g. `github.com/scverse/scirpy/.github/workflows/release.yaml@refs/tags/v0.24.0`.
...
`precommit-pinned` is red almost everywhere because the template itself pins pre-commit hooks to tags.
The template is what needs changing.

`zizmor-clean` runs the real [zizmor](https://docs.zizmor.sh) over the workflows, composite actions and Dependabot config the collector already fetched.
It audits everything zizmor can read rather than the paths a repo's own hook is scoped to, which is how this repo once shipped an unaudited `dependabot.yml`.
A repo's `# zizmor: ignore[…]` comments and `.github/zizmor.yml` are honoured, and the `regular` persona keeps false positives out of a required check.
Findings need only `contents:read`; with a token the online audits for stale and known-vulnerable action refs run too, and the cell says which of the two it was.

### Branch protection & review

Rulesets (`GET /repos/{o}/{r}/rules/branches/{branch}`) are the primary source and need only a read token.
Repos still on *classic* branch protection return `[]` there, so these fall back to `/branches/{b}/protection`, which needs `administration:read` — hence `admin?`, and hence `?` rather than a failure when the App is absent.
...
The cron authenticates as the **`scverse-repo-health` GitHub App**, installed on the scverse org with **read-only** permissions.
...
The workflow inputs follow the REST API rather than the UI labels.
The two that catch people out: "Dependabot alerts" is `vulnerability-alerts`, and check runs come under `checks`, **not** `actions`.

The two lists must agree.
`permission-*:` can only *narrow* what the installation holds, so asking for anything it lacks fails the run with a 422 that does not name the culprit.
If you drop a permission from the App, drop the matching input too.

> **Adding a permission to an App does not reach existing installations.**
> GitHub queues it for review and the installation keeps its old set until an org owner accepts.
> If you granted Checks after installing and still get a 422, that acceptance is what is missing: **Organization settings → GitHub Apps → `scverse-repo-health` → Review request**.
...
It also works with no credentials at all, subject to the 60 requests/hour anonymous limit.
...
Then add a fixture-backed test in [`tests/`](tests/).
Fixtures are snapshots of real repositories, captured with:

```console
$ uv run python scripts/capture_fixture.py scirpy scanpy
```

[`config/core_devs.yaml`](config/core_devs.yaml) is a hand-maintained list of the core team, a GitHub `login` and a `name` per person.
It is committed rather than read from [scverse.org/people](https://scverse.org/people) at run time, so the dashboard does not depend on that page's structure.
Keep it in step with the website by hand.
